...
Model_parameters = ['size', 'init_sheep', 'init_wolf', 'regrow_time', 'sheep_gain', 'sheep_reproduce_rate', 'wolf_gain', 'wolf_reproduce_rate', 'seed']

agent_class_components = [['Sheep', ['AgentComponent', 'SpeciesComponent']], ['Wolf', ['AgentComponent', 'SpeciesComponent']]]

model_flexible_parameters = [['system', 'food', [['regrow_time', 'int']]], ['agent', ['Sheep', 'SpeciesComponent'], [['sheep_gain', 'int', 'gain'], ['sheep_reproduce_rate', 'float', 'reproduce_rate']]], ['agent', ['Wolf', 'SpeciesComponent'], [['wolf_gain', 'int', 'gain'], ['wolf_reproduce_rate', 'float', 'reproduce_rate']]]]

Re_init_parameters = ['size', 'seed', 'sheep_gain', 'sheep_reproduce_rate', 'wolf_gain', 'wolf_reproduce_rate']

# ModelRegion

class PredatorPreySheep(Core.Model):
    def __init__(self, size: int, init_sheep: int, init_wolf: int, regrow_time: int, sheep_gain: int, sheep_reproduce_rate: float, wolf_gain: int, wolf_reproduce_rate: float, seed: int = None):
        ...
        self.model_name = 'PredatorPreySheep'
    ...
    # Create Agents At Random Locations
        for _ in range(init_sheep):
            self.environment.add_agent(Sheep(self), x_pos=self.random.randint(0, size - 1), y_pos=self.random.randint(0, size - 1))
        for _ in range(init_wolf):
            self.environment.add_agent(Wolf(self), x_pos=self.random.randint(0, size - 1), y_pos=self.random.randint(0, size - 1))
...
# -----------------------------------------------------------------------------------------------------------------------
# This commented by ''' ''' section is for the user to run the model outside the graphical IDE, and show outputs.
# To use the function, un-comment this section.
...
if __name__ == "__main__":
    ...
    PredatorPreySheep.PredatorPreySheep(init_sheep: int, init_wolf: int, regrow_time: int, sheep_gain: int, sheep_reproduce_rate: float, wolf_gain: int, wolf_reproduce_rate: float, )

    # User needs to define the size for x and y limits according to model size
    ax2.set_xlim(0, size)
    ax2.set_ylim(0, size)
